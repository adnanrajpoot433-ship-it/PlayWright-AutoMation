...
- Location: tests\accessibilityTesting.spec.ts:21:5
...
```ts
  ...
  20 | 
  21 | test("Acessibility test", async({page})=>{
  22 | 
  23 | await page.goto("https://demowebshop.tricentis.com/");
  24 | 
  25 | // checking that web is following the accesbility or not 
  26 | 
  27 | //1) scanning detect all types of WCAG violations
  28 | 
  29 |  //await new AxeBuilder({page}).analyze(); // will scan the page and analyze all the issue and return the json format
  30 |  
  31 |  const accesbilityScanResult=await new AxeBuilder({page}).analyze();
  32 | 
  33 |  console.log("Number of violations:",accesbilityScanResult.violations.length);
  34 | 
  35 |  //expect(accesbilityScanResult.violations).toEqual([]) ;// violations will collect the violations from the result
  36 |   
> 37 |  expect(accesbilityScanResult.violations.length).toEqual(0);
     |                                                  ^ Error: expect(received).toEqual(expected) // deep equality
  38 | 
  39 | })
```